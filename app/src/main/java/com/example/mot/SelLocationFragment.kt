...
class SelLocationFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onCreateView(
        ...
        val disInfo = arguments
        if (disInfo != null) {
            binding.locationStr.text = disInfo.getString("locationInfo")
        }
        ...
        // 수정한 교통 정보 적용
        val trafInfo = arguments
        if (trafInfo != null) {
            binding.tvTrafficWay.text = trafInfo.getString("trafficInfo")
        }
        ...
        // 수정한 위치 정보 적용
        val adrInfo = arguments
        if (adrInfo != null) {
            binding.adressDetail.text = adrInfo.getString("addressInfo")
        }
